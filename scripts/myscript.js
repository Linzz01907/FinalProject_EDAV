...
d3.select("body").append("p")
   ...
   .text("In this bubble plot, each circle represents a location, with the circle's size indicating the salary amount. Larger circles correspond to higher salaries. Hovering the mouse over a circle displays the salary for that specific area. A summary of the locations and their respective salaries is available in the upper right corner, accessible by clicking the button above.");
...
// Positioning for button and table
...
var button = d3.select("body").append("button")
               .attr("style", "position: absolute; top: " + margin.top + "px; right: " + margin.right + "px;")
               .text("Click to display the salary sorting table")
               .on("click", function() {
                 if (d3.select("table").empty()) {
                   // Create a table element and the header
                   var table = d3.select("body").append("table")
                                 .attr("style", "position: absolute; top: " + (margin.top + buttonHeight + 5) + "px; right: " + margin.right + "px;");
